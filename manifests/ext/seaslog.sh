php_iv_ext_manifest_load() {
  ...
  PHP_IV_EXT_REASON="seaslog is not supported for this PHP version in the current manifest."
  ...
  if php_iv_version_in_range "$php_version" "7.0.0" "8.4.99"; then
    ...
    PHP_IV_EXT_PACKAGE_VERSION="1.10.2"
    PHP_IV_EXT_SOURCE_ARCHIVE="seaslog-1.10.2.tgz"
    PHP_IV_EXT_SOURCE_URL="https://pecl.php.net/get/seaslog-1.10.2.tgz"
    PHP_IV_EXT_SOURCE_DIR="seaslog-1.10.2"
    PHP_IV_EXT_SUPPORTED="1"
    PHP_IV_EXT_REASON=""
  fi
}

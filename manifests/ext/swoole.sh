php_iv_ext_manifest_load() {
  local php_version="$1"

  PHP_IV_EXT_NAME="swoole"
  PHP_IV_EXT_PACKAGE_VERSION=""
  PHP_IV_EXT_SOURCE_ARCHIVE=""
  PHP_IV_EXT_SOURCE_URL=""
  PHP_IV_EXT_SOURCE_DIR=""
  PHP_IV_EXT_SUPPORTED="0"
  PHP_IV_EXT_REASON="swoole is not supported for this PHP version in the current manifest."
  PHP_IV_EXT_CONFIGURE_ARGS=()
  PHP_IV_EXT_INI_LINES=("extension=swoole.so")
  PHP_IV_EXT_NOTES="PECL swoole release pinned by manifest."

  if php_iv_version_in_range "$php_version" "8.0.0" "8.4.99"; then
    PHP_IV_EXT_PACKAGE_VERSION="6.1.6"
    PHP_IV_EXT_SOURCE_ARCHIVE="swoole-6.1.6.tgz"
    PHP_IV_EXT_SOURCE_URL="https://pecl.php.net/get/swoole-6.1.6.tgz"
    PHP_IV_EXT_SOURCE_DIR="swoole-6.1.6"
    PHP_IV_EXT_SUPPORTED="1"
    PHP_IV_EXT_REASON=""
  elif php_iv_version_in_range "$php_version" "7.0.0" "7.4.99"; then
    PHP_IV_EXT_PACKAGE_VERSION="4.4.26"
    PHP_IV_EXT_SOURCE_ARCHIVE="swoole-4.4.26.tgz"
    PHP_IV_EXT_SOURCE_URL="https://pecl.php.net/get/swoole-4.4.26.tgz"
    PHP_IV_EXT_SOURCE_DIR="swoole-4.4.26"
    PHP_IV_EXT_SUPPORTED="1"
    PHP_IV_EXT_REASON=""
  fi
}
